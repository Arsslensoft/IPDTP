...
namespace IPDTPLib
{
	...
	public sealed class PipeManager : IChannelManager {
		...
		private const int MAX_READ_BYTES = IPDTPRules.MaxPacketSize;
		...
		internal string PipeName = "upl://128.215.52";
		...
		public string HandleRequest(string request) {
            ...
            IPDTPApplication.ChatHistory.Add(request);
			...
			try {
				while (_listen) {
					...
					if (numChannels <= NumberPipes) {
						...
						try {
                            pipe.PacketReceived += new PacketReceivedHandler(pipe_PacketReceived);
							pipe.Connect();
